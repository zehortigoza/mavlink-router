...
void Mainloop::router_msg(struct buffer *buf, int target_sysid, int sender_sysid)
{
    if (target_sysid > 0) {
        ...
        if (target) {
            log_info("Routing message from %u to endpoint %u[%d]",
                     sender_sysid, target_sysid, target->fd);
        ...
        log_info("Routing message from %u to all other known endpoints",
                 sender_sysid);
...
int Mainloop::tcp_open(unsigned long tcp_port)
{
    ...
    log_info("Open TCP 0.0.0.0:%lu", tcp_port);
