...
// 1 minute o authorization
#define AUTHORIZATION_TIMEOUT_MSEC (1000 * 60)
...
static void command_ack2_send(mavlink_command_ack2_t *ack)
{
    mavlink_message_t msg;
    mavlink_msg_command_ack2_encode(SYSTEM_ID, MAV_COMP_ID_ALL, &msg, ack);
    msg_send(&msg);
}
...
static void mission_request_int_send(uint16_t seq)
{
    ...
    mission_req.mission_type = MAV_MISSION_TYPE_ALL;
...
static void mission_ack_send()
{
    ...
    mission_ack.type = 0;
    mission_ack.mission_type = MAV_MISSION_TYPE_ALL;

    mavlink_msg_mission_ack_encode(SYSTEM_ID, MAV_COMP_ID_ALL, &msg, &mission_ack);
    msg_send(&msg);
}

static void handle_command_long(mavlink_command_long_t *cmd)
{
    ...
    count_req.mission_type = MAV_MISSION_TYPE_ALL;
    mavlink_msg_mission_request_list_encode(SYSTEM_ID, MAV_COMP_ID_ALL, &msg, &count_req);
    msg_send(&msg);
...
static void handle_mission_item_int(mavlink_mission_item_int_t *mission_item)
{
    ...
    if (mission_item->seq == mission_item_count) {
        ...
        if (mission_valid) {
            ...
            ack.result_param1 = AUTHORIZATION_TIMEOUT_MSEC;
            ack.result_param2 = 0;
            printf("Arm request accepted\n");
        } else {
            ack.result = MAV_RESULT_DENIED;
            ack.result_param1 = MAV_ARM_AUTH_DENIED_REASON_INVALID_WAYPOINT;
    ...
    while (!g_should_exit) {
        ...
        if (state != STATE_IDLE) {
            if (timespec_compare(&now, &auth_request_timeout) > 0) {
                ...
                ack.result_param1 = MAV_ARM_AUTH_DENIED_REASON_TIMEOUT;
                ack.result_param2 = 0;
